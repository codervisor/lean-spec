...
| Layer                | Parent Cycle | Self-Reference | Hierarchy/Dep Conflict |
| -------------------- | ------------ | -------------- | ---------------------- |
| `lean-spec validate` | ✅ Full       | ✅              | ❌                      |
| MCP `set_parent`     | ⚠️ Self only  | ✅              | ❌                      |
| MCP `relationships`  | ❌            | ❌              | ❌                      |
| CLI `rel add`        | ❌            | ❌              | ❌                      |

This allows invalid states:
1. **Parent cycles**: `A→B→C→A`
2. **Hierarchy/dependency conflicts**: A spec that `depends_on` its own parent or children
...
2. **Hierarchy/dependency conflict detection** - Prevent overlapping relationships
...
2. **MCP `tool_relationships`** (action=add, type=parent or depends_on)
3. **CLI `rel add`** - rust/leanspec-cli/src/commands/rel.rs
...
/// Check if adding dependency is valid
