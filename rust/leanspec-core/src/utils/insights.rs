...
use crate::types::{SpecInfo, SpecStatus, SpecPriority};
use crate::utils::SpecStats;
...
impl Insights {
        ...
        let high_priority_planned: Vec<_> = specs
            ...
            .filter(|s| {
                s.frontmatter.status == SpecStatus::Planned
                    && matches!(s.frontmatter.priority, Some(SpecPriority::High) | Some(SpecPriority::Critical))
            })
            .map(|s| s.path.clone())
            .collect();
        ...
        let no_assignee: Vec<_> = specs
            ...
            .filter(|s| {
                s.frontmatter.status == SpecStatus::InProgress
                    && s.frontmatter.assignee.is_none()
            })
            .map(|s| s.path.clone())
            .collect();
        ...
        if !stale.is_empty() {
            self.messages.push(InsightMessage {
                ...
                message: format!(
                    ...
                    stale.len(), stale_threshold_days
                ),
                related_specs: stale,
            });
        }
            ...
            .filter(|s| {
                s.frontmatter.status == SpecStatus::InProgress
                    && s.frontmatter.depends_on.iter().any(|dep| {
                        all_paths.contains(dep) && !complete_paths.contains(dep)
                    })
            })
            .map(|s| s.path.clone())
            .collect();
    ...
    pub fn has_issues(&self) -> bool {
        self.messages.iter().any(|m| {
            matches!(m.severity, InsightSeverity::Warning | InsightSeverity::Critical)
        })
    }
}
...
mod tests {
    ...
    fn test_high_priority_planned() {
        let specs = vec![
            create_spec("001-urgent", SpecStatus::Planned, Some(SpecPriority::Critical), None),
        ];
    ...
    fn test_in_progress_no_assignee() {
        let specs = vec![
            create_spec("001-feature", SpecStatus::InProgress, None, None),
        ];
        
        let stats = SpecStats::compute(&specs);
        let insights = Insights::generate(&specs, &stats);
        
        assert!(insights.messages.iter().any(|m| m.message.contains("assignee")));
    }
}
