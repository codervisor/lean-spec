...
impl SpecStats {
    ...
    pub fn completion_percentage(&self) -> f64 {
        ...
        let complete = self.by_status.get(&SpecStatus::Complete).copied().unwrap_or(0);
        (complete as f64 / self.total as f64) * 100.0
    }
    
    /// Get active specs (planned + in-progress)
    pub fn active_count(&self) -> usize {
        let planned = self.by_status.get(&SpecStatus::Planned).copied().unwrap_or(0);
        let in_progress = self.by_status.get(&SpecStatus::InProgress).copied().unwrap_or(0);
        planned + in_progress
    }
...
mod tests {
    ...
    fn create_spec(status: SpecStatus, priority: Option<SpecPriority>, tags: Vec<&str>) -> SpecInfo {
        SpecInfo {
            path: "test".to_string(),
    ...
    fn test_compute_stats() {
        let specs = vec![
            create_spec(SpecStatus::Planned, Some(SpecPriority::High), vec!["feature"]),
            create_spec(SpecStatus::InProgress, Some(SpecPriority::Medium), vec!["feature", "cli"]),
