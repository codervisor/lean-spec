...
use leanspec_core::{SpecLoader, DependencyGraph};
use std::error::Error;
...
pub fn run(
    ...
    if output_format == "json" {
        ...
        let output = DepsOutput {
            ...
            depends_on: if downstream_only { Vec::new() } else {
                graph.get_upstream(&spec_info.path, depth).iter().map(to_dep_info).collect()
            },
            required_by: if upstream_only { Vec::new() } else {
                graph.get_downstream(&spec_info.path, depth).iter().map(to_dep_info).collect()
            },
    ...
    if !downstream_only {
        ...
        println!("{} {} (what this spec needs)", "⬆️".bold(), "Depends On".bold());
        ...
        if upstream.is_empty() && complete.depends_on.is_empty() {
            ...
            for dep in &complete.depends_on {
                ...
                println!("   {} {} - {}", status_emoji, dep.path.cyan(), dep.title.dimmed());
            }
            ...
            if depth > 1 && upstream.len() > complete.depends_on.len() {
                ...
                for dep in &upstream {
                    if !complete.depends_on.iter().any(|d| d.path == dep.path) {
                        let status_emoji = dep.frontmatter.status_emoji();
                        println!("     {} {} - {}", status_emoji, dep.path, dep.title.dimmed());
                    }
                }
            }
        }
    ...
    if !upstream_only {
        ...
        println!("{} {} (what needs this spec)", "⬇️".bold(), "Required By".bold());
