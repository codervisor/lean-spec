...
pub fn run(
    ...
    if let Some(spec_path) = spec {
        ...
        if output_format == "json" {
            ...
            let (total_tokens, optimal_count, good_count, warning_count, excessive_count) =
                results.iter().fold(
                    (0usize, 0usize, 0usize, 0usize, 0usize),
                    |(total, opt, good, warn, exc), (_, r)| {
                        use leanspec_core::utils::TokenStatus;
                        let new_opt = if matches!(r.status, TokenStatus::Optimal) {
                            opt + 1
                        } else {
                            opt
                        };
                        let new_good = if matches!(r.status, TokenStatus::Good) {
                            good + 1
                        } else {
                            good
                        };
                        let new_warn = if matches!(r.status, TokenStatus::Warning) {
                            warn + 1
                        } else {
                            warn
                        };
                        let new_exc = if matches!(r.status, TokenStatus::Excessive) {
                            exc + 1
                        } else {
                            exc
                        };
            ...
            let output = TokensOutput {
                ...
                summary: Summary {
                    ...
                    total_tokens,
                    average_tokens: if results.is_empty() {
                        0
                    } else {
        ...
        let optimal = results
            .iter()
            .filter(|(_, r)| matches!(r.status, TokenStatus::Optimal | TokenStatus::Good))
            .count();
        let warning = results
            .iter()
            .filter(|(_, r)| matches!(r.status, TokenStatus::Warning))
            .count();
        let excessive = results
            .iter()
            .filter(|(_, r)| matches!(r.status, TokenStatus::Excessive))
            .count();
