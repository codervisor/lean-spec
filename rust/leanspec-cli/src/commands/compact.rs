...
fn parse_removes(removes: &[String]) -> Result<Vec<RemoveSpec>, Box<dyn Error>> {
    ...
    for remove in removes {
        ...
        if parts.len() != 2 {
            return Err(format!("Invalid line range format: {}. Expected format: '145-153'", remove).into());
        }
...
fn display_dry_run(
    ...
    for remove in removes {
        ...
        println!("  Lines {}-{} ({} lines)", remove.start, remove.end, line_count);
        ...
        if remove.end - remove.start + 1 > 3 {
            println!("      {} ({} more lines)", "...".dimmed(), remove.end - remove.start - 2);
        }
        println!();
    }
    ...
    println!("  Removing:        {} lines ({}%)", total_removed.to_string().yellow(), percentage);
...
fn execute_compact(
    ...
    for remove in removes {
        let line_count = remove.end - remove.start + 1;
        println!("{} Removed lines {}-{} ({} lines)", "✓".green(), remove.start, remove.end, line_count);
    }
    ...
    println!("Removed {} lines ({}%)", removed_count.to_string().dimmed(), percentage);
