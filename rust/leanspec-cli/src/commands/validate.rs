...
use leanspec_core::{
    SpecLoader, DependencyGraph, 
    FrontmatterValidator, StructureValidator, LineCountValidator,
    ValidationResult,
};
use std::error::Error;
...
pub fn run(
    ...
    let specs_to_validate = if let Some(spec_path) = spec {
        let spec = loader.load(&spec_path)?
...
fn print_json(results: &[ValidationResult]) -> Result<(), Box<dyn Error>> {
    ...
    let output: Vec<_> = results.iter().map(|r| JsonResult {
        spec: r.spec_path.clone(),
        issues: r.issues.iter().map(|i| JsonIssue {
...
fn print_text(results: &[ValidationResult], total_specs: usize) {
    if results.is_empty() {
        println!("{} All {} specs passed validation", "✓".green(), total_specs);
        return;
    }
    ...
    println!(
        ...
        total_specs,
        if total_errors > 0 { total_errors.to_string().red() } else { "0".green() },
        if total_warnings > 0 { total_warnings.to_string().yellow() } else { "0".green() }
    );
}
