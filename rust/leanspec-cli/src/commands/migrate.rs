...
fn detect_source_format(documents: &[DocumentInfo]) -> String {
    ...
    let has_spec_kit = documents.iter().any(|d| 
        d.path.contains(".specify") || d.name == "spec.md"
    );
    if has_spec_kit {
...
fn migrate_auto(
    ...
    for doc in documents {
        ...
        let parent_name = doc_path.parent()
            .and_then(|p| p.file_name())
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_else(|| doc.name.replace(".md", ""));
        
        // Skip container folders
        if ["specs", "archive", "changes", "openspec", "node_modules"].contains(&parent_name.as_str()) {
            continue;
        }
...
fn migrate_with_ai(
    ...
    println!("{}", "⚠ AI-assisted migration is not yet fully implemented".yellow());
...
fn output_manual_instructions(
    ...
    println!("  {}", format!("lean-spec migrate {} --auto", input_path).cyan());
    ...
    println!("   {}", "lean-spec update <name> --priority <priority>".cyan());
    println!();
    ...
    println!("{}", "─".repeat(70));
    println!();
    println!("{} Use {} for automated migration", "ℹ".cyan(), "--auto".cyan());
    println!();
}
