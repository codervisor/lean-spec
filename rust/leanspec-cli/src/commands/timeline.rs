...
pub fn run(
    ...
    // Take last N months
    let months_vec: Vec<_> = by_month.iter()
        .rev()
        .take(months)
        .collect();
    ...
    println!("  Specs created: {}", total_created.to_string().green().bold());
    println!("  Specs completed: {}", total_completed.to_string().green().bold());
