...
use leanspec_core::{SpecLoader, SpecStatus, DependencyGraph};
use std::error::Error;
...
pub fn run(
    ...
    let mut sorted_specs: Vec<_> = filtered.iter()
        .filter_map(|s| {
            ordered_paths.iter()
                .position(|p| p == &s.path)
                .map(|pos| (pos, *s))
    ...
    if output_format == "json" {
        ...
        let output = Output {
            specs: sorted_specs.iter()
                .enumerate()
                .map(|(i, (_, s))| {
                    // Find incomplete dependencies that block this spec
                    let blocked_by: Vec<_> = s.frontmatter.depends_on.iter()
                        .filter(|dep| {
                            specs.iter()
                                .find(|sp| &sp.path == *dep)
                ...
                .collect(),
            execution_order: sorted_specs.iter()
    ...
    let max_title_len = sorted_specs.iter()
    ...
    for (i, (_, spec)) in sorted_specs.iter().enumerate() {
        ...
        // Check if blocked
        let blocked_by: Vec<_> = spec.frontmatter.depends_on.iter()
            .filter(|dep| {
                specs.iter()
                    .find(|s| &s.path == *dep)
        ...
        if !spec.frontmatter.depends_on.is_empty() {
            let deps_str = spec.frontmatter.depends_on.iter()
                .map(|d: &String| {
    ...
    println!();
    println!("{}", "Legend:".bold());
    println!("  {} Complete  {} In Progress  {} Planned",
        "████████".green(),
        "████░░░░".yellow(),
        "░░░░░░░░".blue()
    );
    ...
    // Summary
    let complete = sorted_specs.iter().filter(|(_, s)| s.frontmatter.status == SpecStatus::Complete).count();
    let in_progress = sorted_specs.iter().filter(|(_, s)| s.frontmatter.status == SpecStatus::InProgress).count();
    let planned = sorted_specs.iter().filter(|(_, s)| s.frontmatter.status == SpecStatus::Planned).count();
    let blocked: usize = sorted_specs.iter()
        .filter(|(_, s)| {
    ...
    println!("Summary: {} complete, {} in progress, {} planned, {} blocked",
