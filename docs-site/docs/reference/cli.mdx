...
Launch a local web UI for visual spec browsing and management.
...
**Options:**
- `-s, --specs <dir>` - Specs directory (auto-detected if not specified)
...
- 🎯 Kanban board view
- 🔗 Relationship visualization (dependencies, related specs)
- 📱 Responsive design
- ⚡ Real-time updates (changes appear within 60 seconds)

**How It Works:**

The UI uses **filesystem mode** for local projects:
...
If you don't specify `--specs`, the UI automatically looks for specs in:
1. `./specs`
2. `./spec`
3. `./docs/specs`
4. `./docs/spec`
...
```bash
# Initialize LeanSpec first
lean-spec init

# Or specify directory explicitly
lean-spec ui --specs ./my-custom-dir
```
...
- Use the URL printed in terminal to open manually
- Check `--no-open` flag wasn't set accidentally

**Use Cases:**
- Visual overview of project status
- Onboarding new team members
- Presentations and demos
- Quick navigation between related specs
