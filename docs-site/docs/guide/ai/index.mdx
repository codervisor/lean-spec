...
When properly integrated, AI agents use LeanSpec specs in two ways:

### AI as Implementer (Traditional)
...
### AI as Co-Author (New)
1. **Draft Specs** - Create initial spec structure from human intent
2. **Expand Sections** - Turn brief notes into full descriptions
3. **Generate Examples** - Create test cases and usage examples
4. **Format Content** - Organize and structure information
5. **Iterate with Human** - Refine based on feedback and refinement

**The Shift:** AI doesn't just consume specs—it helps create them. This makes specs more accessible while maintaining quality through human review.
...
See the [AGENTS.md Guide](/docs/guide/ai/agents-md) for a complete template.
...
│   ├── 001-feature-name/  # Flat structure (default)
│   │   └── README.md      # The spec
│   ├── 002-another-feature/
│   └── archived/
├── .lspec/
...
```
Implement the feature described in specs/001-user-auth/README.md
```

Or reference multiple specs:

```
Check specs/001-user-auth and specs/002-api-gateway 
before implementing the login endpoint.
```

**Tip**: With default flat structure. If using date-grouping, paths would be `specs/YYYYMMDD/NNN-name/`.
...
See [Setup Guide](/docs/guide/ai/setup) for details.
...
```
Read the spec in specs/001-user-authentication/README.md
...
:::tip Let AI Draft, Human Refine
AI is great at structure and expansion, but tends toward verbosity. Use AI to create initial drafts, then ruthlessly edit for clarity and conciseness.
:::

:::tip Prompt for Intent, Not Implementation
When asking AI to draft specs, focus prompts on "why" and "what," not "how." Example: "Draft a spec for API rate limiting to prevent abuse" not "Draft a spec using Redis with sliding window algorithm."
:::

:::tip Review Against First Principles
After AI drafts a spec, check it against LeanSpec principles: Is it under 400 lines? Does every sentence inform a decision? Is the "why" clear? Is there unnecessary verbosity?
:::

:::tip Iterate Through Conversation
Don't accept the first AI draft. Converse, refine, ask for specific sections to be rewritten. "Make the Overview section more concise" or "Expand the Trade-offs section with more detail."
:::
...
Add comments linking code to specs: `// Implements: specs/001-user-auth`
:::
...
### Pattern 1: AI-Assisted Spec Writing (New)

**The collaborative authoring workflow:**

1. **Converse with AI** - Discuss intent, explore options, clarify requirements
2. **Ask AI to draft spec** - "Create a LeanSpec spec for [feature]"
3. **Review and refine** - Cut verbosity, ensure intent is clear, verify LeanSpec principles
4. **Checkpoint agreement** - Spec becomes contract before implementation
5. **AI implements** - AI builds from refined spec
6. **Update iteratively** - Keep spec in sync with learnings

**When to use:**
- Complex features that benefit from structured thinking
- When conversation has clarified the approach
- Multiple stakeholders need alignment
- Decisions need documentation

**Key practices:**
- Keep AI prompts focused on intent and constraints
- Review AI output critically (AI tends toward verbosity)
- Ensure "why" is captured, not just "what"
- Use LeanSpec principles as editing criteria

**Example prompt:**
```
I need to add rate limiting to our API. We've discussed:
- 100 requests/minute per user
- 1000 requests/hour per IP
- Redis for tracking
- 429 status on limit exceeded

Draft a LeanSpec spec capturing this design. Keep it under 300 lines, 
focus on intent and trade-offs, not implementation details.
```
...
### Pattern 4: Reverse Spec Writing
...
See [Examples](/docs/guide/ai/examples) for real-world AI integration patterns and workflows.

---

**Next**: 
- [Setup Guide](/docs/guide/ai/setup) - Detailed setup instructions
- [AGENTS.md Template](/docs/guide/ai/agents-md) - Complete template
