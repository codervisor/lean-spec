...
4. **Test with AI Agent**
   ...
   Prompt your AI: "Read specs/001-my-feature/README.md and implement it"
...
│       └── spec-template.md
├── specs/
│   ├── 001-first-feature/     # Flat structure (default)
│   ├── 002-second-feature/
│   └── archived/
├── AGENTS.md
└── ... (your code)
...
**Note**: Default structure is flat with global numbering. For date-grouped organization (`YYYYMMDD/NNN-name/`), configure `structure.pattern` in `.lspec/config.json`.
...
This creates:
```
specs/001-user-authentication/README.md
```

**Note**: With default flat structure. If you configured date-grouping, it would be `specs/YYYYMMDD/001-user-authentication/README.md`.
...
```
Read specs/001-user-authentication/README.md and implement 
...
4. Update status: `lspec update 001 --status=in-progress`
...
**Tip**: You can use spec number (001), name (user-authentication), or full path.
...
```
Implement the feature described in specs/001-test-spec
```
